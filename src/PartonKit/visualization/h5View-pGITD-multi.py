...
parser.add_option("-p", "--boostMoms", type="str", default='',
                  help='Fin/Ini momenta of rpGITD numerator <pf1/pi1_pf2/pi2_...>=X.X.X/X.X.X (default = '')')
...
# Momenta to plot
PFPI=[ tuple(pfpi.split('/')) for pfpi in options.boostMoms.split('_') ]
...
pgitdData=gpd_utils.h5Plot(options.h5File,options.dtypeName,options.fit2pt,cfgs,\
                           PFPI[0][0],PFPI[0][1],zmin,zmax,options.Lx)
pgitdData.initAxes()
pgitdData.read2pts()
pgitdData.showPGITDData()

pgitdDataAlts=[gpd_utils.h5Plot(options.h5File,options.dtypeName,options.fit2pt,cfgs,\
                                pf,pi,zmin,zmax,options.Lx) for pf,pi in PFPI[1:]]

for n,G in enumerate(pgitdDataAlts):
    G.initAxes(formAxes=False)
    G.read2pts()
    G.showPGITDData(useAlternateAxis=pgitdData.ampDict,symbOverride=symbs[n])
...
for amp,comps in pgitdData.ampDict.items():
    for fig, ax in [tuple([figAx['fig'],figAx['ax']]) for comp,figAx in comps.items()]:
        ...
        yrange=ax.get_ylim()[1]-ax.get_ylim()[0]
        xtent=ax.get_xlim()[1]-ax.get_xlim()[0]
        # Add dummy points to indicate what momenta are present
        foo=PFPI[0][0]+'.'+PFPI[0][1]
        ax.errorbar(ax.get_xlim()[0]+0.1*xtent,ax.get_ylim()[0]+0.1*yrange,\
                    xerr=0.15,color='gray',marker='o')
        ax.text(ax.get_xlim()[0]+0.1*xtent,ax.get_ylim()[0]+0.1*yrange,\
                r'$\vec{p}_f=\left(%s,%s,%s\right)\ \vec{p}_i=\left(%s,%s,%s\right)$'\
                %tuple(foo.split('.')),fontsize=18)

        for n in range(1,len(PFPI)):
            foo=PFPI[n][0]+'.'+PFPI[n][1]
            ax.errorbar(ax.get_xlim()[0]+0.1*xtent,ax.get_ylim()[0]+0.1*yrange-0.025*n,\
                        xerr=0.15,color='gray',marker='o')
            ax.text(ax.get_xlim()[0]+0.1*xtent,ax.get_ylim()[0]+0.1*yrange-0.025*n,\
                    r'$\vec{p}_f=\left(%s,%s,%s\right)\ \vec{p}_i=\left(%s,%s,%s\right)$'\
                    %tuple(foo.split('.')),fontsize=18)
            # ax.errorbar(0,0.5-0.05*n,xerr=0.15,color='gray',marker=symbs[n-1])
            # ax.text(0,0.5-0.05*n,\
            #         r'$\vec{p}_f=\left(%s,%s,%s\right)\ \vec{p}_i=\left(%s,%s,%s\right)$'\
            #         %tuple(foo.split('.')),fontsize=18)
        
        # # Add dummy points to indicate what momenta are present
        # print(tuple(PFPI[0][0].split('.')))
        # print(tuple(PFPI[0][1].split('.')))
        # ax.errorbar(0,0.5,xerr=0.15,color='gray',marker='o')
        # ax.text(0,0.5,r'$\vec{p}_f=\left(%s,%s,%s\right)\ \vec{p}_i=\left(%s,%s,%s\right)$'\
        #         %(tuple(PFPI[0][0].split('.')),tuple(PFPI[0][1].split('.'))))
        
        # for n in range(1,len(PFPI)):
        #     ax.errorbar(0,0.5-0.05*n,xerr=0.15,color='gray',marker=symbs[n])
        #     ax.text(0,0.5-0.05*n,\
        #             r'$\vec{p}_f=\left(%s,%s,%s\right)\ \vec{p}_i=\left(%s,%s,%s\right)$'\
        #             %(tuple(PFPI[n][0]),tuple(PFPI[n][1])))
