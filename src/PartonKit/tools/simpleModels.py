...
from util.pdf_util import pdf2

def hyp_0F1(n,arg,trunc=100):
    hyp = 0.0
    for k in range(0,trunc+1):
        hyp += (1.0*np.power(arg,k))/(spec.gamma(n+k)*spec.factorial(k))
    return hyp
...
def func(nus,xi,a,b,N):
    ...
    for nu in nus:
        real = lambda beta : np.cos(nu*beta)*forwardPDF.pdf(beta)*\
            hyp_0F1(N+1.5,-0.25*nu**2*xi**2*np.power(1-abs(beta),2))*spec.gamma(N+1.5);
